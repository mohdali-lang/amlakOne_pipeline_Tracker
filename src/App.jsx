  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 10, marginTop: 4, flexWrap: "wrap" }}>
        ...
        <Stat label="Avg KPI today" value={`${avgAchv}%`} tone={toneFor(avgAchv)} />
        <Stat label="Support flags" value={rows.reduce((s, r) => s + r.flags, 0)} sub="deals needing you" tone={C.amber} />
      </div>

      <SectionTitle>Agent scorecard · {today}</SectionTitle>
      {rows.length === 0 && <Empty text="No agents assigned to your team yet." />}
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {rows.map((r) => (
          <div key={r.a.id} style={{ background: C.surf, border: `1px solid ${C.line}`, borderRadius: 12, padding: "12px 14px" }}>
